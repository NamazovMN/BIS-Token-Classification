...
class Train:
    ...
    def __init__(self, config_params: dict, model: ClassifierModel):
        ...
        self.loss = self.set_loss_fn(config_params)
        self.configuration = self.set_configuration(config_params)

    @staticmethod
    def set_loss_fn(parameters: dict) -> nn.CrossEntropyLoss:
        """
        ...
        """
        return nn.CrossEntropyLoss(ignore_index=parameters['label_dict']['<PAD>'])
    ...
    def train_epoch(self, data_loader: dict, max_label: str) -> None:
        ...
        train_range = range(last_epoch + 1 if not last_epoch == 0 else last_epoch, self.configuration['num_epochs'])
        ...
        for epoch in train_range:
            ...
            ti = tqdm(iterable=data_loader['train'], total=num_batches,
                      desc=f'Epoch: {epoch}, TRAIN -> epoch loss: {epoch_loss}, accuracy : {accuracy}')
            print(f'{20 * "<"} TRAIN {20 * ">"}')
            total_tokens = 0
            epoch_accuracy = 0
            for batch in ti:
